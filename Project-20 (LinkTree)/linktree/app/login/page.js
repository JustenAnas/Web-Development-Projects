...
const Login = () => {
  ...
  const handleLogin = (e) => {
    ...
    if (!email.includes("@") || !email.split("@")[1]?.includes("gmail.com")) {
      console.log("Invalid email");
      setError(
        "Invalid email! It should include '@' and a valid domain like 'gmail.com'"
      );
    ...
    // Store session or token (use localStorage or cookies in real-world)
    localStorage.setItem("user", JSON.stringify({ email }));

    // Redirect to the Username page after successful login
  ...
  return (
    <div className="text-white bg-slate-400 pt-40 container mx-auto">
      ...
      <div className="login-btn">
        <main className="flex items-center justify-center text-white">
          <section className="w-[25rem] flex flex-col space-y-8 p-6 bg-gray-700 bg-opacity-5 rounded-md shadow-lg">
            <div className="text-center text-3xl font-semibold">Log In</div>

            {/* Email Input */}
            <div className="w-full border-b-2 border-gray-700 focus-within:border-indigo-500 transition duration-300">
              <input
                type="email"
                id="email"
                name="email"
                ...
                className="w-full py-2 bg-transparent text-white placeholder-yellow-300 focus:outline-none"
                required
            ...
            {/* Login Button */}
            <button
              className="w-full py-2 text-lg font-semibold text-white bg-indigo-600 rounded-md transition duration-300 hover:bg-indigo-500 focus:outline-none"
              onClick={handleLogin}
            >
              LOG IN
            </button>
            <a
              ...
              className="text-center font-medium text-white hover:text-gray-300 transition duration-300"
            >
              FORGOT PASSWORD?
            </a>
            ...
            <p className="text-center text-base">
              ...
              <a
                href="/signup"
                className="font-medium text-indigo-500 ml-1 hover:underline"
              >
                Create One
              </a>
            </p>
      ...
      <div className="bg-gray-400 bg-opacity-5 text-center font-medium text-2xl text-white mx-auto w-[25rem]">
        or
      </div>

      <div className="flex items-center flex-col gap-3 p-10">
        <button
          className="flex items-center justify-center w-80 bg-white border border-gray-300 rounded-lg shadow-md max-w-xs px-6 py-2 text-[17px] font-medium text-gray-800 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500"
          onClick={() => {
